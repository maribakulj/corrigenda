...
Complements ``test_orchestrator_snapshot`` (which pins the contract at
end-to-end granularity via a snapshot of structural facts). This file
pins the same invariant at rewriter-call granularity so a regression
fails closer to the offending change.

If a future change leaks into the UNTOUCHED path and silently rewrites
a stable line, the corresponding assertion fires.
"""
...
def test_identity_corrections_take_untouched_or_subs_only_paths():
    ...
    The FAST_PATH or SLOW_PATH appearing here would mean the rewriter
