"""Security tests for alto-core XML parsing — XXE / SSRF / entity expansion.

Roadmap L10 Phase 1 (B1) — these tests pin the contract that EVERY
lxml parser instantiation in alto-core goes through the hardened
``make_safe_parser()`` helper in ``alto_core.alto._ns``.
...
from alto_core.alto.rewriter import extract_output_texts
...
# ---------------------------------------------------------------------------
# Source-AST contract pin — every lxml call site under alto_core/alto/
...
def test_no_alto_lxml_call_site_uses_default_parser():
    ...
    src_root = Path(__file__).resolve().parents[1] / "src" / "alto_core" / "alto"
...
def test_make_safe_parser_enables_all_four_safety_flags():
    ...
    ns_path = (
        Path(__file__).resolve().parents[1] / "src" / "alto_core" / "alto" / "_ns.py"
    )
