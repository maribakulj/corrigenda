...
def main() -> int:
    ...
    # The contract is `corrigenda.__all__`: every name listed there MUST
    ...
    for name in corrigenda.__all__:
        if not hasattr(corrigenda, name):
            missing.append(name)
            continue
        if getattr(corrigenda, name) is None:
            none_valued.append(name)

    if missing or none_valued:
        print(
            f"corrigenda smoke FAILED for version {corrigenda.__version__}",
            file=sys.stderr,
        )
        if missing:
    ...
    print(
        f"smoke ok: corrigenda {corrigenda.__version__} "
        f"({len(corrigenda.__all__)} public symbols)"
    )
    return 0
