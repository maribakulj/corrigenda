...
``runner.py`` documents the contract:
...
``test_sanitize_error.py`` pins this at the helper level. This file
pins it at the runner's *caller* level: a real exception escaping the
pipeline with an api_key sitting around the 500-char boundary must
produce a sanitised, truncated ``job.error`` without a partial leak.
...
async def test_runner_sanitizes_short_message_with_key(
    ...
    """Baseline: a short message. Re-asserting it here keeps the
    boundary test honest — if both ordering pass, the sanitization is
    genuinely happening at the caller."""
    api_key = "sk-secret-token-12345678"
