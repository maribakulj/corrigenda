...
# Event names known to be emitted by the backend on representative runs.
# Pinned here so a CI failure clearly identifies WHICH event drifted —
# rather than just "frontend missing some event".
_KNOWN_BACKEND_EVENTS = {
    ...
    "keepalive",
    "error",
    # Frontend-only initial state — kept here so the set comparison stays
    # symmetric and the assertion message is meaningful.
    "queued",
}
...
# ---------------------------------------------------------------------------
# Static check: parse worked and known events are non-empty
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# The contract: every known backend event must be listed by the frontend
# ---------------------------------------------------------------------------


def test_known_backend_events_are_in_frontend_contract():
    """Pin the symmetric relationship: every event the backend can emit
    must appear in the frontend EVENTS list. A new backend event without
    the corresponding frontend update is silently dropped — this test
    breaks the silence."""
    missing = _KNOWN_BACKEND_EVENTS - FRONTEND_EVENTS
    assert not missing, (
        f"Frontend EVENTS list is missing {sorted(missing)}. "
        f"Either add them to frontend/src/hooks/useJobStream.ts::EVENTS "
        f"or remove the backend emit site if the event is no longer needed."
    )
...
async def test_happy_path_emits_only_events_in_frontend_contract(tmp_path: Path):
    """A real run must only emit names the frontend knows how to
    dispatch — concrete check on top of the static one above."""
