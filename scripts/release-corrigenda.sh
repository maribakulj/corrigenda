#!/usr/bin/env bash
# Release script for packages/corrigenda.
#
# Usage:
#   scripts/release-corrigenda.sh             # build + smoke-install only
#   scripts/release-corrigenda.sh --testpypi  # build + upload to TestPyPI
#   scripts/release-corrigenda.sh --pypi      # build + upload to PyPI (prod)
#
# Pre-flight (manual):
#   1. Bump src/corrigenda/__init__.py::__version__
...
# .github/workflows/publish-corrigenda.yml for the CI path.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PKG_DIR="${REPO_ROOT}/packages/corrigenda"
TARGET=""
...
init = pathlib.Path('src/corrigenda/__init__.py').read_text()
...
if not m:
    sys.exit('__version__ not found in src/corrigenda/__init__.py')
print(m.group(1))
")
echo "==> Releasing corrigenda ${VERSION}"
...
# with .github/workflows/{ci,publish-corrigenda}.yml — see roadmap
...
INSTALLED_VERSION=$("${SMOKE_VENV}/bin/python" -c "import corrigenda; print(corrigenda.__version__)")
...
if [[ "${TARGET}" == "testpypi" ]]; then
    INDEX_JSON_URL="https://test.pypi.org/pypi/corrigenda/json"
else
    INDEX_JSON_URL="https://pypi.org/pypi/corrigenda/json"
fi
INDEX_TMP="$(mktemp)"
...
"; then
            echo "ERROR: corrigenda ${VERSION} is already on ${TARGET}." >&2
            echo "       Bump src/corrigenda/__init__.py::__version__ and add a CHANGELOG entry." >&2
            rm -f "${INDEX_TMP}"
            exit 1
        fi
        ...
        ;;
    404)
        echo "==> Index check: corrigenda not yet published on ${TARGET}, first release."
        ;;
    000)
...
if [[ "${TARGET}" == "testpypi" ]]; then
    ...
    echo "==> Verify at https://test.pypi.org/project/corrigenda/${VERSION}/"
else
    echo "==> Uploading to PyPI (production)"
    read -r -p "Confirm production upload of corrigenda ${VERSION}? [y/N] " confirm
    if [[ "${confirm}" != "y" && "${confirm}" != "Y" ]]; then
        echo "Aborted."
        exit 1
    fi
    python -m twine upload dist/*
    echo "==> Verify at https://pypi.org/project/corrigenda/${VERSION}/"
fi
