...
5. `core/pipeline.py` — `CorrectionPipeline`, the engine. Retry logic (3 attempts per chunk, then granularity downgrade, then fallback to OCR source text), guards (`core/guards.py`), edit protocol (`core/editing.py`), cooperative cancellation via `should_abort`. **One instance = one run at a time; `run()` mutates the input manifest**
