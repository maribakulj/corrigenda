...
- **NB2** (L6, closed L8) — 7 of the 8 backend shims (`app/alto/parser.py`, `rewriter.py`, `hyphenation.py`, `_norm.py`, `app/jobs/chunk_planner.py`, `validator.py`, `line_acceptance.py`) used `# noqa: F401  re-export` instead of declaring an explicit `__all__` like `correction_pipeline.py` does. The latter is the better pattern: it makes the re-export surface a first-class object linters can reason about, and it's robust against ruff's RUF100 (which strips redundant `noqa` comments). **Resolution:** L8 migrated all 7 stragglers to explicit `__all__`. The post-L8 corrective wave subsequently DELETED `app/alto/_norm.py` and `app/alto/_ns.py` outright (commit `618be08`, attributed below to the L8 corrective wave / S6) since they had zero consumers.
...
| P1       | `c678ab2`    | P1-1        | fix(frontend): wrap addFiles in useCallback (superseded by `7bd3d4e`) |
| P1       | `4e9caa0`    | NB1         | chore(frontend): bump vite to 6.4.2 and postcss to 8.5.15 |
| P2       | `8e4eace`    | docs        | refresh stale NB2 entry + close R5 as not-a-bug |
| P1       | `7bd3d4e`    | P1-1 (v2)   | refactor(frontend): functional setState instead of useCallback chain |
| P0       | (this push)  | self-audit  | reframe R3 + fix L8/L9 attribution + correct NB1 description |

### Items closed in L9

- **R3** — `JobStore.stream_events` checked status FIRST, then called `subscribe()`. **Honesty caveat:** under the current single-threaded asyncio model the race CANNOT fire — `emit()` is only invoked from coroutines on the same event loop as `stream_events`, and there is no `await` between the original status check and `subscribe()`, so no other coroutine can interleave. The fix is forward-looking hardening for any future refactor that moves emission to a worker thread, a multi-process queue, an `asyncio.to_thread`-style callback, or a multi-worker uvicorn deployment with a shared distributed JobStore. Reordered to subscribe first, then re-check status; the post-subscribe check drains anything already in the queue and falls back to a synthetic terminal event if nothing arrived. Backed by `test_stream_events_does_not_lose_terminal_during_subscribe_race`, which monkey-patches `subscribe` to simulate the race; the test pins the contract "consumer always gets a terminal event under this race scenario" rather than reproducing a bug observable in today's code. Verified to FAIL against the pre-fix code (consumer times out at 2 s) and PASS after.
- **P1-1** — ESLint `react-hooks/exhaustive-deps` warning on `FileUpload.tsx:54`. Initial fix (commit `c678ab2`) wrapped `addFiles` in useCallback and chained it through `onDrop`; the post-L9 hostile review correctly flagged this as bricolage — the chain produced a new identity on every `files` change anyway, giving zero memoisation benefit. **Replaced by commit `7bd3d4e`**: refactored both `addFiles` and `removeFile` to use functional `setFiles((prev) => ...)` updates, so the handlers no longer close over `files`. Net result: no useCallback, no eslint-disable, no stale-closure risk, -10 lines. `npm run lint` clean.
- **NB1** — `vite ≤ 6.4.1` (HIGH: path traversal + arbitrary file read) and `postcss < 8.5.10` (MODERATE: XSS) flagged by `npm audit` since L1. Both bumps (vite 6.4.1 → 6.4.2, postcss 8.4.49 → 8.5.15, plus an incidental transitive `nanoid` 3.3.11 → 3.3.12 from the postcss bump) fall inside existing `^` ranges so only `package-lock.json` moves. `package.json` keeps `"vite": "^6.0.5"` and `"postcss": "^8.4.49"`; lockfile pinning is what makes `npm ci` reproducible — a future `rm package-lock.json && npm install` could re-resolve to a fresh CVE. `npm audit` post-bump: 0 vulnerabilities.
